...
```ts
function all<T extends readonly [...Option<unknown>[]]>(xs: readonly [...T]): Option<TypeOfOptionArray<T>>
```
...
```ts
function contains<A>(option: Option<A>, value: unknown): boolean
function contains<A>(value: unknown): (option: Option<A>) => boolean
```
...
```ts
function fold<A, B>(option: Option<A>, someFn: (value: A) => B, errorFn: () => B): B
function fold<A, B>(someFn: (value: A) => B, errorFn: () => B): (option: Option<A>) => B
```
...
```ts
function fromExecution<A>(fn: () => A): Option<NonNullable<A>>
```
...
```ts
function fromFalsy<A>(value: A): Option<NonNullable<A>>
```
...
```ts
function fromNullable<A>(value: A): Option<NonNullable<A>>
```
...
```ts
function fromPredicate<A>(value: A, predicateFn: (value: NonNullable<A>) => boolean): Option<NonNullable<A>>
function fromPredicate<A>(predicateFn: (value: NonNullable<A>) => boolean): (value: A) => Option<NonNullable<A>>
```
...
```ts
function fromPromise<A>(promise: Promise<A>): Promise<Option<NonNullable<A>>>
```
...
```ts
function getWithDefault<A extends NonNullable<unknown>>(option: Option<A>, defaultValue: A): A
function getWithDefault<A extends NonNullable<unknown>>(defaultValue: A): (option: Option<A>) => A
```
...
```ts
function makeNone<A>(): Option<NonNullable<A>>
```
...
```ts
function makeSome<A>(value: NonNullable<A>): Option<A>
```
...
```ts
function mapNullable<A, B>(option: Option<A>, mapFn: (value: A) => B | null | undefined): Option<NonNullable<B>>
function mapNullable<A, B>(mapFn: (value: A) => B | null | undefined): (option: Option<A>) => Option<NonNullable<B>>
```
...
```ts
function mapWithDefault<A, B extends NonNullable<unknown>>(option: Option<A>, defaultValue: B, mapFn: (value: A) => B): B
function mapWithDefault<A, B extends NonNullable<unknown>>(defaultValue: B, mapFn: (value: A) => B): (option: Option<A>) => B
```
...
```ts
function toResult<A, B>(option: Option<A>, errorValue: B): Result<A, B>
function toResult<A, B>(errorValue: B): (option: Option<A>) => Result<A, B>
```
