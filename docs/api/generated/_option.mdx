...
```ts
function getWithDefault<A extends NonNullable<unknown>>(option: Option<A>, defaultValue: NoInfer<A>): A
function getWithDefault<A extends NonNullable<unknown>>(defaultValue: NoInfer<A>): (option: Option<A>) => A
```
