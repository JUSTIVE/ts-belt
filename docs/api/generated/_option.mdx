...
```ts
function all<T extends readonly [...Option<any>[]]>(xs: readonly [...T]): Option<TypeOfOptionArray<T>>
```
...
```ts
function getWithDefault<A extends NonNullable<any>>(option: Option<A>, defaultValue: A): A
function getWithDefault<A extends NonNullable<any>>(defaultValue: A): (option: Option<A>) => A
```
...
```ts
function mapWithDefault<A, B extends NonNullable<any>>(option: Option<A>, defaultValue: B, mapFn: (value: A) => B): B
function mapWithDefault<A, B extends NonNullable<any>>(defaultValue: B, mapFn: (value: A) => B): (option: Option<A>) => B
```
...
```ts
function toNullable<A>(option: Option<A>): NonNullable<A> | null
```
