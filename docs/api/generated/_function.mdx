...
```ts
function andThen<A, B>(value: A, fn: (value: A) => B): B
function andThen<A, B>(fn: (value: A) => B): (value: A) => B
```
...
```ts
function tryCatch<A, B>(value: A, fn: (value: A) => B): Result<B, globalThis.Error>
function tryCatch<A, B>(fn: (value: A) => B): (value: A) => Result<B, globalThis.Error>
```
...
F.tryCatch('<>', JSON.parse) // → R.Error(new Error('Unexpected token < in JSON at position 0'))
pipe(
  '{"name": "Joe"}',
