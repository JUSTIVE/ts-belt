...
pipe({ name: "Joe", location: "Warsaw" }, D.deleteKeys([])) // → { name: "Joe", location: "Warsaw" }
...
```ts
function get<T, K extends keyof T>(dict: T, key: K): Option<NonNullable<T[K]>>
function get<T, K extends keyof T>(key: K): (dict: T) => Option<NonNullable<T[K]>>
```
...
```ts
function keys<T extends Record<string, unknown>>(dict: T): Array<`${Extract<keyof T, string | number>}`>
```
